...
object_reference_process()는 아래 함수들을 순서대로 부르기만 한다 (실제 로직은 각 함수 안에 있음):
  ...
  filter_candidates_by_relations -> 질문에 "near/between/closest to..." 같은 공간관계가
                                  있으면 랜드마크를 즉석 검출해서 후보를 deterministic하게
                                  좁힌다 (object_filter/candidate_filter.py, SORT-3D Module 3+4)
  select_candidate_among       -> 좁혀진 후보가 1개면 그대로, 여러 개면 그 안에서
                                  Qwen이 최종 선택 (색 등 시각 판단)
...
def object_reference_process(node, question):
    ...
    scan_points_map = get_scan_points_in_map(node, "ObjectRef")

    candidate_indices = filter_candidates_by_relations(
        node, question, detections, image, image_stamp, scan_points_map,
    )
    selected_index = select_candidate_among(node, image, detections, candidate_indices, question)
...
def select_candidate_among(node, image, detections, candidate_indices, question):
    """
    filter_candidates_by_relations()가 좁혀준 후보 안에서 최종 선택한다.

    공간관계로 후보가 1개까지 좁혀졌으면 그대로 쓴다 (Qwen 안 부름 —
    거리/방향 판단은 이미 결정론적으로 끝났으므로). 여러 개 남았으면
    (예: 관계로는 후보가 안 좁혀지거나, 색만 다른 동일 위치 후보들) 그
    안에서만 Qwen이 시각적으로 고른다.
    """
    if len(candidate_indices) == 1:
        node.get_logger().info(f"[ObjectRef] selected by spatial relation, index={candidate_indices[0]}")
        return candidate_indices[0]

    filtered_detections = [detections[i] for i in candidate_indices]
    local_index = select_candidate(node, image, filtered_detections, question)
    return candidate_indices[local_index]
