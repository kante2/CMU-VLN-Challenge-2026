...
object_reference_process()는 아래 함수들을 순서대로 부르기만 한다 (실제 로직은 각 함수 안에 있음):
  ...
  estimate_3d_target          -> 선택 box를 point cloud에 재투영해서 3D 좌표+크기 계산
...
def object_reference_process(node, question):
    ...
    save_projection_debug(node, image, selected.box, scan_points_map, detect_prompt, image_stamp)
...
def estimate_3d_target(node, selected_box, image_size, scan_points_map, detect_prompt, image_stamp=None):
    return box_to_3d(
        selected_box,
        image_size,
        scan_points_map,
...
def save_projection_debug(node, image, box, scan_points_map, detect_prompt, image_stamp):
