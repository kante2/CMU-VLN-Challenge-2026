...
실행 순서 (질문 하나가 처리되는 흐름):
  1. question_callback / image_callback / scan_callback / pose_callback
     (node/callbacks.py) — 전부 "최신값 저장"만 한다.
  ...
  4. t*_*_solver/ 가 실제 검출 -> 선택 -> 3D 위치 -> 발행까지 처리한다.
     ...
     각 스텝 함수는 solver별 context 구조체(node/context.py)를 받아
     자기 필드를 채우고, 다음 함수가 그 필드를 읽어 이어서 쓴다.)

폴더 구조 (도메인별로 묶여 있음):
  node/        노드 뼈대·수명주기. setup(초기화/모델로딩), callbacks(센서콜백),
               helpers(공용 상태 조회), context(solver별 작업변수 ctx 생성 함수)
  ...
  perception/  2D 인식: ROS Image 변환(image_utils), GroundingDINO 검출(detector),
               검출어 추출(query_parser), 디버그 저장(visualize), SAM 마스크(segmenter),
               Qwen 시각 선택(selector)
  geometry/    3D 기하: 좌표 변환(coordinate_transform), 2D->3D 재투영(projector),
               3D bbox 크기/wireframe(bbox_estimator, bbox_wireframe)
...
이 파일 자체는 이제 node 로직을 직접 담지 않고 "조립"만 한다:
  1. Import
  2. Node class: __init__은 node/setup.py의 함수들을 순서대로 호출
  3. Main control loop: main_control_loop() -> dispatch_question()
     (node/helpers.py의 함수를 불러 쓴다)
  4. main()
"""
...
from tmah_vlm.node.setup import (
    initialize_state,
    initialize_modules,
...
from tmah_vlm.node.helpers import (
