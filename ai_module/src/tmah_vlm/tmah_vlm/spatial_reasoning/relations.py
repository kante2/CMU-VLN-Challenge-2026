...
입출력 규약 (이 파일의 모든 함수 공통):
  ...
  (grounding/projector.py의 make_box_candidate_mask 등과 같은 스타일).
  order_*/closest_to/farthest_from은 index(정수)를 반환한다.

  point들의 실제 정체(뭐가 어떤 물체인지)는 이 파일이 모른다 — 호출하는 쪽
  (예: handlers/numerical.py, handlers/object_reference.py)이 candidate 목록과
...
def find_near(points, reference_point, max_distance=None):
    """reference_point에서 max_distance 이내인 point들의 mask."""
    if max_distance is None:
        max_distance = config.SPATIAL_NEAR_THRESHOLD_M
    return distances_to(points, reference_point) <= max_distance
...
def order_by_distance(points, reference_point, descending=False):
    """reference_point 기준 가까운 순(기본) index 배열."""
    order = np.argsort(distances_to(points, reference_point))
...
def closest_to(points, reference_point):
    """가장 가까운 point의 index. point가 없으면 None."""
    points = _as_points(points)
    if points.shape[0] == 0:
        return None
    return int(np.argmin(distances_to(points, reference_point)))


def farthest_from(points, reference_point):
    """가장 먼 point의 index. point가 없으면 None."""
    points = _as_points(points)
    if points.shape[0] == 0:
        return None
    return int(np.argmax(distances_to(points, reference_point)))


def largest_face_area(bbox_size):
    """
    bbox3d/estimator.py가 만든 (sx, sy, sz) 크기에서 가장 넓은 면의 넓이를 구한다.
