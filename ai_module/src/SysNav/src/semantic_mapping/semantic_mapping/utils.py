...
# Resolve objects.yaml next to this module so it works regardless of the launch
# CWD (SysNav is nested under src/SysNav/, so the old CWD-relative path broke).
object_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config', 'objects.yaml')
