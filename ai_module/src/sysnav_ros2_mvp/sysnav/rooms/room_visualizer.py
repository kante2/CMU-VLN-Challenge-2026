...
def export_room_segmentation(
    ...
    for room in result["rooms"]:
        ...
        label = f'Room {room["room_id"]} ({room["cell_count"]} cells)'
        (text_width, text_height), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
        origin = (max(0, cx - text_width // 2), max(text_height, cy))
        cv2.putText(canvas, label, origin, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
