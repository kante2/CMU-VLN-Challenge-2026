...
class VLMNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('object_file', 'src/semantic_mapping/semantic_mapping/config/objects.yaml')
        ...
        self.text_overlay_publisher = self.create_publisher(
            OverlayText, '/vlm_answer', 10
        ) if _HAVE_OVERLAY else None
    ...
    def publish_text_overlay(self, text: str, duration: float = 8.0):
        if not _HAVE_OVERLAY or self.text_overlay_publisher is None:
            return  # RViz overlay unavailable; status text is non-functional
        self._last_msg_id += 1
